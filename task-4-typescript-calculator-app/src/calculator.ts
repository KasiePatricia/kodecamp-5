 ...
 */
const percentage = (value: number, percentage: number): number => (value * percentage) / 100;
...
const promptOperationChoice = (): Promise<string> => {
...
const getTwoNumbers = (): Promise<{ num1: string; num2: string }> => {
    return new Promise((resolve) => {
        rl.question('Enter first number (or type "last" to use previous result): ', (num1: string) => {
...
const getSingleNumber = (): Promise<string> => {
    return new Promise((resolve) => {
        rl.question('Enter number (or "last" to use previous result): ', (num: string) => {
            resolve(num.trim());
        });
    });
}
 ...
 */
const validateNumbers = (num1: string, num2: string): NumbersValidationResult => {
...
async function mainMenuLoop(): Promise<void> {
    ...
    while (true) {
        try {
            ...
            const choice: string = await promptOperationChoice();
            ...
            if (twoNumberOperations.includes(choice as OperationChoice)) {
                const { num1, num2 }: { num1: string; num2: string } = await getTwoNumbers();
